...
#include "cuda_runtime_api.h"
#endif
static int zmumps_arith_threshold;
static int zmumps_arith_threshold_above, zmumps_arith_threshold_under;
static int zmumps_gpu_under_l0;
#if ! defined(USE_XKBLAS)
/* for asynchronous pinning */
# include "mumps_register_thread.h"
static cublasXtHandle_t zmumps_handle;
#endif
#include <mpi.h>
/* dev_version... ok not to prefix the global symbols below with zmumps_
 * since we rarely mix different arithmetics with dev_version */
double zmumps_wtime_pin, zmumps_wtime_async_pin, zmumps_wtime_gemm, zmumps_wtime_gemmt, zmumps_wtime_trsm, zmumps_wtime_unpin,zmumps_wtime_copyscale;
double zmumps_size_pin, zmumps_size_async_pin, zmumps_size_async_unpin, zmumps_flops_gemm, zmumps_flops_gemmt, zmumps_flops_trsm, zmumps_flops_copyscale;
double zmumps_start_async_pin;
/* for L0-OMP: */
double zmumps_wtime_l0_gemm, zmumps_wtime_l0_gemmt, zmumps_wtime_l0_trsm, zmumps_wtime_l0_copyscale;
double zmumps_flops_l0_gemm, zmumps_flops_l0_gemmt, zmumps_flops_l0_trsm, zmumps_flops_l0_copyscale;
#if defined (USE_XKBLAS)
...
{
  return ((2.0 * (double)m * (double)n * (double)k) / ((double)m * (double)n + (double)m * (double)k + (double)n * (double)k) >= (double)zmumps_arith_threshold );
}/* isEnoughWorkGemm */
static inline int isEnoughWorkTrsm(int m, int n, char* side)
{
  ...
  return (comp >= (double)zmumps_arith_threshold );
}/* isEnoughWorkTrsm */
...
{
  return (((double)n * (double)n * (double)k) / ((double)n * (double)n + (double)n * (double)k + (double)n * (double)k )) >= (double)zmumps_arith_threshold;
}/* isEnoughWorkGemmT */
static inline void add_value_to_counter(double *counter_above, double *counter_under, double value)
{
  if (zmumps_gpu_under_l0 == 1)
  {
#if defined(_OPENMP)
#pragma omp atomic
#endif
...
#if defined(USE_XKBLAS)
/* ZMUMPS_INIT_GPU_C: XKBLAS  version. */
void MUMPS_CALL
 ...
 */
{
  ...
  int ierr;
#endif
  zmumps_wtime_pin = 0.0;
  zmumps_wtime_async_pin = 0.0;
  zmumps_wtime_gemm = 0.0;
  zmumps_wtime_trsm = 0.0;
  zmumps_wtime_copyscale = 0.0;
  zmumps_wtime_gemmt = 0.0;
  zmumps_wtime_unpin = 0.0;
  zmumps_flops_gemm = 0.0;
  zmumps_flops_gemmt = 0.0;
  zmumps_flops_trsm = 0.0;
  zmumps_flops_copyscale = 0.0;
  zmumps_size_pin = 0.0;
  zmumps_size_async_pin = 0.0;
  zmumps_size_async_unpin = 0.0;
  /* for L0-OMP: */
  zmumps_wtime_l0_gemm = 0.0;
  zmumps_wtime_l0_trsm = 0.0;
  zmumps_wtime_l0_copyscale = 0.0;
  zmumps_wtime_l0_gemmt = 0.0;
  zmumps_flops_l0_gemm = 0.0;
  zmumps_flops_l0_gemmt = 0.0;
  zmumps_flops_l0_trsm = 0.0;
  zmumps_flops_l0_copyscale = 0.0;
...
#endif
  if (*K417 > 0) /* Note: always the case when ZMUMPS_GPU_INIT_C is called */
  {
#if defined(MUMPS_WIN32) || defined(__MINGW32__)
  ...
  xkblas_set_param( tile_size, sizeof(ZMUMPS_COMPLEX) );
  ret = xkblas_init();
  if (ret != 0)
  {
    info[0]=-105;
  ...
  zmumps_arith_threshold = 0;
    /* Threshold obtained from *K418 */
    if ( *K418 > 0)
    {
      zmumps_arith_threshold = *K418;
    }
  zmumps_arith_threshold_above = zmumps_arith_threshold;
  zmumps_arith_threshold_under = *K415;
  zmumps_gpu_under_l0 = 0; /* L0OMP status */
  return;
}/* ZMUMPS_INT_GPU_C (using XKblas) */
#else
/* ZMUMPS_INIT_GPU_C: CUBLASXT version.
...
/* the number of devices per MPI is bounded by 32 in MUMPS i
  ...
  zmumps_flops_l0_gemmt = 0.0;
  zmumps_flops_l0_trsm = 0.0;
  ret=cublasXtCreate(&zmumps_handle);
  ...
  ret= cublasXtDeviceSelect(zmumps_handle, *K417, devices);
  if(ret != CUBLAS_STATUS_SUCCESS)
  {
    info[0]=-103;
    info[1]=ret;
  ...
  if ( *K419 > 0) 
  {
    ret=cublasXtSetBlockDim(zmumps_handle, *K419);
    if (ret != CUBLAS_STATUS_SUCCESS)
    {
      info[0]=-104;
      info[1]=ret;
  ...
  zmumps_arith_threshold_above = zmumps_arith_threshold;
  zmumps_arith_threshold_under = *K415; /* FIXME: use a KEEP */
  zmumps_gpu_under_l0 = 0; /* L0OMP status */
...
#endif
  return;
}/* ZMUMPS_INIT_GPU_C (using cublasXt) */
#endif
...
{
  *flops_gpu=zmumps_flops_gemm+zmumps_flops_gemmt+zmumps_flops_trsm+zmumps_flops_copyscale;
  *time_gpu=zmumps_wtime_gemm+zmumps_wtime_gemmt+zmumps_wtime_trsm+zmumps_wtime_copyscale;
  if (*K400 > 0)
  {
    /* Take into account L0-threads; time is averaged on all threads */
    *flops_gpu=*flops_gpu+zmumps_flops_l0_gemm+zmumps_flops_l0_gemmt+zmumps_flops_l0_trsm+zmumps_flops_l0_copyscale;
    *time_gpu=*time_gpu+(zmumps_wtime_l0_gemm+zmumps_wtime_l0_gemmt+zmumps_wtime_l0_trsm+zmumps_wtime_l0_trsm) / *K400;
  }
#if defined(USE_XKBLAS)
  xkblas_finalize();
#else
...
#endif
  if(cublasXtDestroy(zmumps_handle) != CUBLAS_STATUS_SUCCESS)
  {
...
ZMUMPS_SYMSCHUR_ON_GPU_C(MUMPS_INT *GPU_ACTIVATION, MUMPS_INT *N, MUMPS_INT *K, MUMPS_INT *BLOCKING)
{
  ...
  if (*GPU_ACTIVATION > 0 && *N > 0 && *K > 0)
  {
    ...
    if (arith_intensity2 >= zmumps_arith_threshold)
    {
      symschur_on_gpu = 1;
      *BLOCKING = blsize;
    }
...
}/* zmumps_need_pinning_C */
#endif
#if defined(USE_XKBLAS)
void MUMPS_CALL
ZMUMPS_GEMM_GPU_C(MUMPS_INT *gpu_async, char* TransA, char* TransB, MUMPS_INT *M, MUMPS_INT *N, MUMPS_INT *K, ZMUMPS_COMPLEX *ALPHA, ZMUMPS_COMPLEX *A,
                  MUMPS_INT *LDA, ZMUMPS_COMPLEX *B, MUMPS_INT *LDB, ZMUMPS_COMPLEX *BETA, ZMUMPS_COMPLEX *C, MUMPS_INT *LDC, mumps_ftnlen l1)
{
  ...
  xkblas_memory_coherent_async(0,0,*M, *N, C, *LDC, sizeof(ZMUMPS_COMPLEX));
  if (*gpu_async == 0)
    {
      xkblas_sync();
  ...
  add_value_to_counter(&zmumps_wtime_gemm, &zmumps_wtime_l0_gemm, elapsed);
  add_value_to_counter(&zmumps_flops_gemm, &zmumps_flops_l0_gemm, flops);
}/* zmumps_gemm_gpu_C (with XKblas) */
#else
void MUMPS_CALL
ZMUMPS_GEMM_GPU_C(MUMPS_INT *gpu_async, char* TransA, char* TransB, MUMPS_INT *M, MUMPS_INT *N, MUMPS_INT *K, ZMUMPS_COMPLEX *ALPHA, ZMUMPS_COMPLEX *A, 
                  MUMPS_INT *LDA, ZMUMPS_COMPLEX *B, MUMPS_INT *LDB, ZMUMPS_COMPLEX *BETA, ZMUMPS_COMPLEX *C, MUMPS_INT *LDC, mumps_ftnlen l1)
{
  ...
  double flops;
  start = MPI_Wtime();
  cublasStatus=cublasXtZgemm(zmumps_handle, op_blas2cublasxt(TransA), op_blas2cublasxt(TransB), *M,*N, *K, (cuDoubleComplex *) ALPHA, (cuDoubleComplex *) A, *LDA, (cuDoubleComplex *) B, *LDB, (cuDoubleComplex *) BETA, (cuDoubleComplex *) C, *LDC);
    /* FIXME: decide if the following error notification should be "public" or what to do with it */
  if(cublasStatus != CUBLAS_STATUS_SUCCESS)
  {
    printf("In ZMUMPS_GEMM_GPU_C, cublasXtZgemm failed with status %i\n",cublasStatus);
  }
  elapsed = MPI_Wtime() - start;
  flops = 2*((double)*M*((double)*N)*((double)*K));
  add_value_to_counter(&zmumps_wtime_gemm, &zmumps_wtime_l0_gemm, elapsed);
  add_value_to_counter(&zmumps_flops_gemm, &zmumps_flops_l0_gemm, flops);
}/* zmumps_gemm_gpu_C (with cublasXt) */
#endif
#if defined(USE_XKBLAS)||defined(GEMMT_AVAILABLE_FOR_GPU)
void MUMPS_CALL
ZMUMPS_GEMMT_GPU_C(MUMPS_INT *gpu_async, char* Uplo, char* TransA, char* TransB, MUMPS_INT *N, MUMPS_INT *K, ZMUMPS_COMPLEX *ALPHA, ZMUMPS_COMPLEX *A, MUMPS_INT *LDA, ZMUMPS_COMPLEX *B, MUMPS_INT *LDB, ZMUMPS_COMPLEX *BETA, ZMUMPS_COMPLEX *C, MUMPS_INT *LDC, mumps_ftnlen l1)
{
  double start, elapsed;
  double flops;
  start = MPI_Wtime();
  xkblas_zgemmt_async(xkblas_blas2cblas_fill(Uplo), xkblas_blas2cblas_trans(TransA), xkblas_blas2cblas_trans(TransB), *N, *K,
    (Complex64_t *)ALPHA, (Complex64_t *)A, *LDA,
                          (Complex64_t *)B, *LDB,
    (Complex64_t *)BETA,  (Complex64_t *)C, *LDC
  );
  xkblas_memory_coherent_async(xkblas_blas2cblas_fill(Uplo), 0, *N, *N, C, *LDC, sizeof(ZMUMPS_COMPLEX));
  xkblas_sync();
  xkblas_memory_invalidate_caches();
  elapsed = MPI_Wtime() - start;
  flops = ((double)*N*((double)*N)*((double)*K));
  add_value_to_counter(&zmumps_wtime_gemmt, &zmumps_wtime_l0_gemmt, elapsed);
  add_value_to_counter(&zmumps_flops_gemmt, &zmumps_flops_l0_gemmt, flops);
}/* zmumps_gemmt_gpu_C (with XKblas) */
#else
void MUMPS_CALL
ZMUMPS_SYRKX_GPU_C(MUMPS_INT *gpu_async, char* Uplo, char* Trans, MUMPS_INT *N, MUMPS_INT *K, ZMUMPS_COMPLEX *ALPHA, ZMUMPS_COMPLEX *A, MUMPS_INT *LDA, ZMUMPS_COMPLEX *B, MUMPS_INT *LDB, ZMUMPS_COMPLEX *BETA, ZMUMPS_COMPLEX *C, MUMPS_INT *LDC, mumps_ftnlen l1)
{
  cublasStatus_t cublasStatus;
  double start, elapsed;
  double flops;
  start = MPI_Wtime();
  cublasStatus=cublasXtZsyrkx(zmumps_handle, fillmode_blas2cublasxt(Uplo), op_blas2cublasxt(Trans), *N, *K, (cuDoubleComplex *) ALPHA, (cuDoubleComplex *) A, *LDA, (cuDoubleComplex *) B, *LDB, (cuDoubleComplex *) BETA, (cuDoubleComplex *) C, *LDC);
 ...
 */
{
  ...
  start = MPI_Wtime();
   M = (*irowmax-*irowmin+1);
   N = *npiv; 
xkblas_zcopyscale_async( (size_t)M, (size_t)N, (*should_copy), &IW[*offset_iw-1],
                         (Complex64_t*)(&A[*dpos-1]), (size_t)(*nfront),
                         (Complex64_t*)(&A[*lpos-1]), (size_t)(*nfront),
                         (Complex64_t*)(&A[*upos-1]), (size_t)(*nfront)
);
  /* L will be needed on CPU */
  xkblas_memory_coherent_async(0, 0, N, M, &A[*lpos-1], *nfront, sizeof(ZMUMPS_COMPLEX));
  /* U: to be checked in which cases CPU really needs it back */
  xkblas_memory_coherent_async(0, 0, M, N, &A[*upos-1], *nfront, sizeof(ZMUMPS_COMPLEX));
  ...
  elapsed = MPI_Wtime() - start;
  flops = M*N;
  add_value_to_counter(&zmumps_wtime_copyscale, &zmumps_wtime_l0_copyscale, elapsed);
  add_value_to_counter(&zmumps_flops_copyscale, &zmumps_flops_l0_copyscale, flops);
} /* ZMUMPS_GPU_COPY2U_SCALEL_C */
#endif /* USE_XKBLAS_COPYSCALE */
void MUMPS_CALL
ZMUMPS_TRSM_GPU_C(MUMPS_INT *gpu_async, char* Side, char* Uplo, char* TransA, char* Diag, MUMPS_INT *M, MUMPS_INT *N,
                  ZMUMPS_COMPLEX *ALPHA, ZMUMPS_COMPLEX *A, MUMPS_INT *LDA, ZMUMPS_COMPLEX *B, MUMPS_INT *LDB, mumps_ftnlen l1)
{
  double start, elapsed;
  double flops;
  start = MPI_Wtime();
  xkblas_ztrsm_async(xkblas_blas2cblas_side(Side), xkblas_blas2cblas_fill(Uplo),
  ...
  if (*gpu_async != 3)
  {
    xkblas_memory_coherent_async(0, 0, *M, *N, B, *LDB, sizeof(ZMUMPS_COMPLEX));
    if (*gpu_async == 0)
    {
      xkblas_sync();
      xkblas_memory_invalidate_caches();
  ...
  add_value_to_counter(&zmumps_wtime_trsm, &zmumps_wtime_l0_trsm, elapsed);
  add_value_to_counter(&zmumps_flops_trsm, &zmumps_flops_l0_trsm, flops);
}/* zmumps_trsm_gpu_C (with XKblas) */
#else
void MUMPS_CALL
                  ...
                  MUMPS_INT *LDA, ZMUMPS_COMPLEX *B, MUMPS_INT *LDB, mumps_ftnlen l1)
{
  ...
  start = MPI_Wtime();
  cublasStatus_t cublasStatus;
  cublasStatus=cublasXtZtrsm(zmumps_handle, sidemode_blas2cublasxt(Side), fillmode_blas2cublasxt(Uplo), op_blas2cublasxt(TransA), diagtype_blas2cublasxt(Diag), *M, *N, (cuDoubleComplex *) ALPHA, (cuDoubleComplex *) A, *LDA, (cuDoubleComplex *) B, *LDB);
  /* FIXME: decide if the following error notification should be "public" or what to do with it */
  if(cublasStatus != CUBLAS_STATUS_SUCCESS)
  {
    printf("In ZMUMPS_TRSM_GPU_C, cublasXtZtrsm failed with status %i\n",cublasStatus);
  }
  elapsed = MPI_Wtime() - start;
  flops = flopsTrsm(*M, *N, Side);
  add_value_to_counter(&zmumps_wtime_trsm, &zmumps_wtime_l0_trsm, elapsed);
  add_value_to_counter(&zmumps_flops_trsm, &zmumps_flops_l0_trsm, flops);
}/* zmumps_trsm_gpu_C (with cublasXt) */
#endif
...
ZMUMPS_GPU_ASYNC_PIN_MEMORY_C(ZMUMPS_COMPLEX *A, MUMPS_INT8 *ADIM)
{
  uint64_t handle;
  double flops;
  zmumps_start_async_pin = MPI_Wtime();
#if defined(DEBUG_GPU)
  printf(" <= GPU_ASYNC_PIN_MEMORY_C: %p of size %li\n",A, *ADIM*sizeof(ZMUMPS_COMPLEX));
#endif
  handle = xkblas_register_memory_async( A, *ADIM*sizeof(ZMUMPS_COMPLEX) );
  flops = *ADIM*sizeof(ZMUMPS_COMPLEX);
#if defined(_OPENMP)
#pragma omp atomic
#endif
  zmumps_size_async_pin = zmumps_size_async_pin + flops;
#if defined(DEBUG_GPU)
  printf(" <= GPU_ASYNC_PIN_MEMORY_C: %p of size %li, handle= %li\n",A, *ADIM*sizeof(ZMUMPS_COMPLEX), handle);
#endif
}/* zmumps_gpu_async_pin_memory_C */
void MUMPS_CALL
...
#pragma omp atomic
#endif
  zmumps_wtime_async_pin = zmumps_wtime_async_pin + elapsed;
  elapsed = stop - zmumps_start_async_pin;
#if defined(_OPENMP)
#pragma omp atomic
#endif
  zmumps_wtime_pin = zmumps_wtime_pin + elapsed;
#if defined(DEBUG_GPU)
...
#endif
  handle = xkblas_unregister_memory_async((void *)A, *ADIM*sizeof(ZMUMPS_COMPLEX));
  double flops = *ADIM*sizeof(ZMUMPS_COMPLEX);
#if defined(_OPENMP)
#pragma omp atomic
#endif
  zmumps_size_async_unpin = zmumps_size_async_unpin + flops;
#if defined(DEBUG_GPU)
  printf(" <= GPU_ASYNC_UNPIN_MEMORY_C: %p of size %li, handle= %li\n",A, *ADIM*sizeof(ZMUMPS_COMPLEX), handle);
#endif
}/* zmumps_gpu_async_unpin_memory_c */
#else
void MUMPS_CALL
ZMUMPS_GPU_ASYNC_PIN_MEMORY_C(ZMUMPS_COMPLEX *A, MUMPS_INT8 *ADIM)
{
  ...
  printf(" => GPU_ASYNC_PIN_MEMORY_C: %p of size %li \n",A, *ADIM*sizeof(ZMUMPS_COMPLEX));
#endif
  handle = mumps_host_register_async((void *)A, *ADIM*sizeof(ZMUMPS_COMPLEX));
  double flops = *ADIM*sizeof(ZMUMPS_COMPLEX);
#if defined(_OPENMP)
#pragma omp atomic
#endif
...
ZMUMPS_GPU_ASYNC_UNPIN_MEMORY_C(ZMUMPS_COMPLEX *A, MUMPS_INT8 *ADIM)
{
  ...
  printf(" => GPU_ASYNC_UNPIN_MEMORY_C: %p of size %li \n",A, *ADIM*sizeof(ZMUMPS_COMPLEX));
#endif
  handle = mumps_host_unregister_async((void *)A, *ADIM*sizeof(ZMUMPS_COMPLEX));
  double flops = *ADIM*sizeof(ZMUMPS_COMPLEX);
#if defined(_OPENMP)
#pragma omp atomic
#endif
...
ZMUMPS_GPU_PIN_MEMORY_C(ZMUMPS_COMPLEX *A, MUMPS_INT8 *ADIM)
{
  ...
  start = MPI_Wtime();
#if defined(USE_XKBLAS)
  error = xkblas_register_memory( A, *ADIM*sizeof(ZMUMPS_COMPLEX) );
  if (error != 0)
  {
...
#else
  error = cudaHostRegister(A, *ADIM*sizeof(ZMUMPS_COMPLEX), cudaHostRegisterPortable);
...
#pragma omp atomic
#endif
  zmumps_size_pin = zmumps_size_pin + flops;
#if defined(DEBUG_GPU)
  printf("<= GPU_PIN_MEMORY_C of %p of size %li, error =%i\n",A, *ADIM*sizeof(ZMUMPS_COMPLEX), error);
#endif
}/* zmumps_gpu_pin_memory_c */
void MUMPS_CALL
ZMUMPS_GPU_MODIFY_ARITH_TRHRESHOLD (MUMPS_INT *OLD, MUMPS_INT *NEW)
{
  /* used to temporarily modify and restore the ARITH_TRHRESHOLD 
     in: NEW value of zmumps_arith_threshold
     out: OLD current value of zmumps_arith_threshold
  */
  *OLD = zmumps_arith_threshold;
  zmumps_arith_threshold = *NEW;
}
void MUMPS_CALL
ZMUMPS_GPU_SET_UNDER_L0_STATUS_C(MUMPS_INT *VALEUR)
{
  /* If zmumps_gpu_under_l0 is 1 it means we are under L0-OMP.
   * If it is 0 it means we are above L0-OMP or that there is no L0-OMP layer.
   */
  zmumps_gpu_under_l0 = *VALEUR;
  if (zmumps_gpu_under_l0 == 0)
  {
    zmumps_arith_threshold = zmumps_arith_threshold_above;
  }
  else
  {
    zmumps_arith_threshold = zmumps_arith_threshold_under;
  }
}
void MUMPS_CALL
ZMUMPS_GPU_UNPIN_MEMORY_C(ZMUMPS_COMPLEX *A, MUMPS_INT8 *ADIM)
{
  ...
  start = MPI_Wtime();
#if defined(USE_XKBLAS)
  error = xkblas_unregister_memory(A, *ADIM*sizeof(ZMUMPS_COMPLEX));
  if (error != 0)
  {
...
#if defined(_OPENMP)
#pragma omp atomic
  zmumps_wtime_unpin = zmumps_wtime_unpin + elapsed;
#endif
#if defined(DEBUG_GPU)
  printf("<= GPU_UNPIN_MEMORY_C of %p of size %li, error=%d\n",A, *ADIM*sizeof(ZMUMPS_COMPLEX),error);
#endif
}/* zmumps_gpu_unpin_memory_c */
#else
