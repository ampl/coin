...
#include "cuda_runtime_api.h"
#endif
static int cmumps_arith_threshold;
static int cmumps_arith_threshold_above, cmumps_arith_threshold_under;
static int cmumps_gpu_under_l0;
#if ! defined(USE_XKBLAS)
/* for asynchronous pinning */
# include "mumps_register_thread.h"
static cublasXtHandle_t cmumps_handle;
#endif
#include <mpi.h>
/* dev_version... ok not to prefix the global symbols below with cmumps_
 * since we rarely mix different arithmetics with dev_version */
double cmumps_wtime_pin, cmumps_wtime_async_pin, cmumps_wtime_gemm, cmumps_wtime_gemmt, cmumps_wtime_trsm, cmumps_wtime_unpin,cmumps_wtime_copyscale;
double cmumps_size_pin, cmumps_size_async_pin, cmumps_size_async_unpin, cmumps_flops_gemm, cmumps_flops_gemmt, cmumps_flops_trsm, cmumps_flops_copyscale;
double cmumps_start_async_pin;
/* for L0-OMP: */
double cmumps_wtime_l0_gemm, cmumps_wtime_l0_gemmt, cmumps_wtime_l0_trsm, cmumps_wtime_l0_copyscale;
double cmumps_flops_l0_gemm, cmumps_flops_l0_gemmt, cmumps_flops_l0_trsm, cmumps_flops_l0_copyscale;
#if defined (USE_XKBLAS)
...
{
  return ((2.0 * (double)m * (double)n * (double)k) / ((double)m * (double)n + (double)m * (double)k + (double)n * (double)k) >= (double)cmumps_arith_threshold );
}/* isEnoughWorkGemm */
static inline int isEnoughWorkTrsm(int m, int n, char* side)
{
  ...
  return (comp >= (double)cmumps_arith_threshold );
}/* isEnoughWorkTrsm */
...
{
  return (((double)n * (double)n * (double)k) / ((double)n * (double)n + (double)n * (double)k + (double)n * (double)k )) >= (double)cmumps_arith_threshold;
}/* isEnoughWorkGemmT */
static inline void add_value_to_counter(double *counter_above, double *counter_under, double value)
{
  if (cmumps_gpu_under_l0 == 1)
  {
#if defined(_OPENMP)
#pragma omp atomic
#endif
...
#if defined(USE_XKBLAS)
/* CMUMPS_INIT_GPU_C: XKBLAS  version. */
void MUMPS_CALL
 ...
 */
{
  ...
  int ierr;
#endif
  cmumps_wtime_pin = 0.0;
  cmumps_wtime_async_pin = 0.0;
  cmumps_wtime_gemm = 0.0;
  cmumps_wtime_trsm = 0.0;
  cmumps_wtime_copyscale = 0.0;
  cmumps_wtime_gemmt = 0.0;
  cmumps_wtime_unpin = 0.0;
  cmumps_flops_gemm = 0.0;
  cmumps_flops_gemmt = 0.0;
  cmumps_flops_trsm = 0.0;
  cmumps_flops_copyscale = 0.0;
  cmumps_size_pin = 0.0;
  cmumps_size_async_pin = 0.0;
  cmumps_size_async_unpin = 0.0;
  /* for L0-OMP: */
  cmumps_wtime_l0_gemm = 0.0;
  cmumps_wtime_l0_trsm = 0.0;
  cmumps_wtime_l0_copyscale = 0.0;
  cmumps_wtime_l0_gemmt = 0.0;
  cmumps_flops_l0_gemm = 0.0;
  cmumps_flops_l0_gemmt = 0.0;
  cmumps_flops_l0_trsm = 0.0;
  cmumps_flops_l0_copyscale = 0.0;
...
#endif
  if (*K417 > 0) /* Note: always the case when CMUMPS_GPU_INIT_C is called */
  {
#if defined(MUMPS_WIN32) || defined(__MINGW32__)
  ...
  xkblas_set_param( tile_size, sizeof(CMUMPS_COMPLEX) );
  ret = xkblas_init();
  if (ret != 0)
  {
    info[0]=-105;
  ...
  cmumps_arith_threshold = 0;
    /* Threshold obtained from *K418 */
    if ( *K418 > 0)
    {
      cmumps_arith_threshold = *K418;
    }
  cmumps_arith_threshold_above = cmumps_arith_threshold;
  cmumps_arith_threshold_under = *K415;
  cmumps_gpu_under_l0 = 0; /* L0OMP status */
  return;
}/* CMUMPS_INT_GPU_C (using XKblas) */
#else
/* CMUMPS_INIT_GPU_C: CUBLASXT version.
...
/* the number of devices per MPI is bounded by 32 in MUMPS i
  ...
  cmumps_flops_l0_gemmt = 0.0;
  cmumps_flops_l0_trsm = 0.0;
  ret=cublasXtCreate(&cmumps_handle);
  ...
  ret= cublasXtDeviceSelect(cmumps_handle, *K417, devices);
  if(ret != CUBLAS_STATUS_SUCCESS)
  {
    info[0]=-103;
    info[1]=ret;
  ...
  if ( *K419 > 0) 
  {
    ret=cublasXtSetBlockDim(cmumps_handle, *K419);
    if (ret != CUBLAS_STATUS_SUCCESS)
    {
      info[0]=-104;
      info[1]=ret;
  ...
  cmumps_arith_threshold_above = cmumps_arith_threshold;
  cmumps_arith_threshold_under = *K415; /* FIXME: use a KEEP */
  cmumps_gpu_under_l0 = 0; /* L0OMP status */
...
#endif
  return;
}/* CMUMPS_INIT_GPU_C (using cublasXt) */
#endif
...
{
  *flops_gpu=cmumps_flops_gemm+cmumps_flops_gemmt+cmumps_flops_trsm+cmumps_flops_copyscale;
  *time_gpu=cmumps_wtime_gemm+cmumps_wtime_gemmt+cmumps_wtime_trsm+cmumps_wtime_copyscale;
  if (*K400 > 0)
  {
    /* Take into account L0-threads; time is averaged on all threads */
    *flops_gpu=*flops_gpu+cmumps_flops_l0_gemm+cmumps_flops_l0_gemmt+cmumps_flops_l0_trsm+cmumps_flops_l0_copyscale;
    *time_gpu=*time_gpu+(cmumps_wtime_l0_gemm+cmumps_wtime_l0_gemmt+cmumps_wtime_l0_trsm+cmumps_wtime_l0_trsm) / *K400;
  }
#if defined(USE_XKBLAS)
  xkblas_finalize();
#else
...
#endif
  if(cublasXtDestroy(cmumps_handle) != CUBLAS_STATUS_SUCCESS)
  {
...
CMUMPS_SYMSCHUR_ON_GPU_C(MUMPS_INT *GPU_ACTIVATION, MUMPS_INT *N, MUMPS_INT *K, MUMPS_INT *BLOCKING)
{
  ...
  if (*GPU_ACTIVATION > 0 && *N > 0 && *K > 0)
  {
    ...
    if (arith_intensity2 >= cmumps_arith_threshold)
    {
      symschur_on_gpu = 1;
      *BLOCKING = blsize;
    }
...
}/* cmumps_need_pinning_C */
#endif
#if defined(USE_XKBLAS)
void MUMPS_CALL
CMUMPS_GEMM_GPU_C(MUMPS_INT *gpu_async, char* TransA, char* TransB, MUMPS_INT *M, MUMPS_INT *N, MUMPS_INT *K, CMUMPS_COMPLEX *ALPHA, CMUMPS_COMPLEX *A,
                  MUMPS_INT *LDA, CMUMPS_COMPLEX *B, MUMPS_INT *LDB, CMUMPS_COMPLEX *BETA, CMUMPS_COMPLEX *C, MUMPS_INT *LDC, mumps_ftnlen l1)
{
  ...
  xkblas_memory_coherent_async(0,0,*M, *N, C, *LDC, sizeof(CMUMPS_COMPLEX));
  if (*gpu_async == 0)
    {
      xkblas_sync();
  ...
  add_value_to_counter(&cmumps_wtime_gemm, &cmumps_wtime_l0_gemm, elapsed);
  add_value_to_counter(&cmumps_flops_gemm, &cmumps_flops_l0_gemm, flops);
}/* cmumps_gemm_gpu_C (with XKblas) */
#else
void MUMPS_CALL
CMUMPS_GEMM_GPU_C(MUMPS_INT *gpu_async, char* TransA, char* TransB, MUMPS_INT *M, MUMPS_INT *N, MUMPS_INT *K, CMUMPS_COMPLEX *ALPHA, CMUMPS_COMPLEX *A, 
                  MUMPS_INT *LDA, CMUMPS_COMPLEX *B, MUMPS_INT *LDB, CMUMPS_COMPLEX *BETA, CMUMPS_COMPLEX *C, MUMPS_INT *LDC, mumps_ftnlen l1)
{
  ...
  double flops;
  start = MPI_Wtime();
  cublasStatus=cublasXtCgemm(cmumps_handle,op_blas2cublasxt(TransA), op_blas2cublasxt(TransB), *M,*N, *K, (cuComplex *) ALPHA, (cuComplex *) A, *LDA, (cuComplex *) B, *LDB, (cuComplex *) BETA, (cuComplex *) C, *LDC);
    /* FIXME: decide if the following error notification should be "public" or what to do with it */
  if(cublasStatus != CUBLAS_STATUS_SUCCESS)
  {
    printf("In CMUMPS_GEMM_GPU_C, cublasXtCgemm failed with status %i\n",cublasStatus);
  }
  elapsed = MPI_Wtime() - start;
  flops = 2*((double)*M*((double)*N)*((double)*K));
  add_value_to_counter(&cmumps_wtime_gemm, &cmumps_wtime_l0_gemm, elapsed);
  add_value_to_counter(&cmumps_flops_gemm, &cmumps_flops_l0_gemm, flops);
}/* cmumps_gemm_gpu_C (with cublasXt) */
#endif
#if defined(USE_XKBLAS)||defined(GEMMT_AVAILABLE_FOR_GPU)
void MUMPS_CALL
CMUMPS_GEMMT_GPU_C(MUMPS_INT *gpu_async, char* Uplo, char* TransA, char* TransB, MUMPS_INT *N, MUMPS_INT *K, CMUMPS_COMPLEX *ALPHA, CMUMPS_COMPLEX *A, MUMPS_INT *LDA, CMUMPS_COMPLEX *B, MUMPS_INT *LDB, CMUMPS_COMPLEX *BETA, CMUMPS_COMPLEX *C, MUMPS_INT *LDC, mumps_ftnlen l1)
{
  double start, elapsed;
  double flops;
  start = MPI_Wtime();
  xkblas_cgemmt_async(xkblas_blas2cblas_fill(Uplo), xkblas_blas2cblas_trans(TransA), xkblas_blas2cblas_trans(TransB), *N, *K,
    (Complex32_t *)ALPHA, (Complex32_t *)A, *LDA,
                          (Complex32_t *)B, *LDB,
    (Complex32_t *)BETA,  (Complex32_t *)C, *LDC
  );
  xkblas_memory_coherent_async(xkblas_blas2cblas_fill(Uplo), 0, *N, *N, C, *LDC, sizeof(CMUMPS_COMPLEX));
  xkblas_sync();
  xkblas_memory_invalidate_caches();
  elapsed = MPI_Wtime() - start;
  flops = ((double)*N*((double)*N)*((double)*K));
  add_value_to_counter(&cmumps_wtime_gemmt, &cmumps_wtime_l0_gemmt, elapsed);
  add_value_to_counter(&cmumps_flops_gemmt, &cmumps_flops_l0_gemmt, flops);
}/* cmumps_gemmt_gpu_C (with XKblas) */
#else
void MUMPS_CALL
CMUMPS_SYRKX_GPU_C(MUMPS_INT *gpu_async, char* Uplo, char* Trans, MUMPS_INT *N, MUMPS_INT *K, CMUMPS_COMPLEX *ALPHA, CMUMPS_COMPLEX *A, MUMPS_INT *LDA, CMUMPS_COMPLEX *B, MUMPS_INT *LDB, CMUMPS_COMPLEX *BETA, CMUMPS_COMPLEX *C, MUMPS_INT *LDC, mumps_ftnlen l1)
{
  cublasStatus_t cublasStatus;
  double start, elapsed;
  double flops;
  start = MPI_Wtime();
  cublasStatus=cublasXtCsyrkx(cmumps_handle, fillmode_blas2cublasxt(Uplo), op_blas2cublasxt(Trans), *N, *K, (cuComplex *) ALPHA, (cuComplex *) A, *LDA, (cuComplex *) B, *LDB, (cuComplex *) BETA, (cuComplex *) C, *LDC);
 ...
 */
{
  ...
  start = MPI_Wtime();
   M = (*irowmax-*irowmin+1);
   N = *npiv; 
xkblas_ccopyscale_async( (size_t)M, (size_t)N, (*should_copy), &IW[*offset_iw-1],
                         (Complex32_t*)(&A[*dpos-1]), (size_t)(*nfront),
                         (Complex32_t*)(&A[*lpos-1]), (size_t)(*nfront),
                         (Complex32_t*)(&A[*upos-1]), (size_t)(*nfront)
);
  /* L will be needed on CPU */
  xkblas_memory_coherent_async(0, 0, N, M, &A[*lpos-1], *nfront, sizeof(CMUMPS_COMPLEX));
  /* U: to be checked in which cases CPU really needs it back */
  xkblas_memory_coherent_async(0, 0, M, N, &A[*upos-1], *nfront, sizeof(CMUMPS_COMPLEX));
  ...
  elapsed = MPI_Wtime() - start;
  flops = M*N;
  add_value_to_counter(&cmumps_wtime_copyscale, &cmumps_wtime_l0_copyscale, elapsed);
  add_value_to_counter(&cmumps_flops_copyscale, &cmumps_flops_l0_copyscale, flops);
} /* CMUMPS_GPU_COPY2U_SCALEL_C */
#endif /* USE_XKBLAS_COPYSCALE */
void MUMPS_CALL
CMUMPS_TRSM_GPU_C(MUMPS_INT *gpu_async, char* Side, char* Uplo, char* TransA, char* Diag, MUMPS_INT *M, MUMPS_INT *N,
                  CMUMPS_COMPLEX *ALPHA, CMUMPS_COMPLEX *A, MUMPS_INT *LDA, CMUMPS_COMPLEX *B, MUMPS_INT *LDB, mumps_ftnlen l1)
{
  double start, elapsed;
  double flops;
  start = MPI_Wtime();
  xkblas_ctrsm_async(xkblas_blas2cblas_side(Side), xkblas_blas2cblas_fill(Uplo),
  ...
  if (*gpu_async != 3)
  {
    xkblas_memory_coherent_async(0, 0, *M, *N, B, *LDB, sizeof(CMUMPS_COMPLEX));
    if (*gpu_async == 0)
    {
      xkblas_sync();
      xkblas_memory_invalidate_caches();
  ...
  add_value_to_counter(&cmumps_wtime_trsm, &cmumps_wtime_l0_trsm, elapsed);
  add_value_to_counter(&cmumps_flops_trsm, &cmumps_flops_l0_trsm, flops);
}/* cmumps_trsm_gpu_C (with XKblas) */
#else
void MUMPS_CALL
                  ...
                  MUMPS_INT *LDA, CMUMPS_COMPLEX *B, MUMPS_INT *LDB, mumps_ftnlen l1)
{
  ...
  start = MPI_Wtime();
  cublasStatus_t cublasStatus;
  cublasStatus=cublasXtCtrsm(cmumps_handle, sidemode_blas2cublasxt(Side), fillmode_blas2cublasxt(Uplo), op_blas2cublasxt(TransA), diagtype_blas2cublasxt(Diag), *M, *N, (cuComplex *) ALPHA, (cuComplex *) A, *LDA, (cuComplex *) B, *LDB);
  /* FIXME: decide if the following error notification should be "public" or what to do with it */
  if(cublasStatus != CUBLAS_STATUS_SUCCESS)
  {
    printf("In CMUMPS_TRSM_GPU_C, cublasXtCtrsm failed with status %i\n",cublasStatus);
  }
  elapsed = MPI_Wtime() - start;
  flops = flopsTrsm(*M, *N, Side);
  add_value_to_counter(&cmumps_wtime_trsm, &cmumps_wtime_l0_trsm, elapsed);
  add_value_to_counter(&cmumps_flops_trsm, &cmumps_flops_l0_trsm, flops);
}/* cmumps_trsm_gpu_C (with cublasXt) */
#endif
...
CMUMPS_GPU_ASYNC_PIN_MEMORY_C(CMUMPS_COMPLEX *A, MUMPS_INT8 *ADIM)
{
  uint64_t handle;
  double flops;
  cmumps_start_async_pin = MPI_Wtime();
#if defined(DEBUG_GPU)
  printf(" <= GPU_ASYNC_PIN_MEMORY_C: %p of size %li\n",A, *ADIM*sizeof(CMUMPS_COMPLEX));
#endif
  handle = xkblas_register_memory_async( A, *ADIM*sizeof(CMUMPS_COMPLEX) );
  flops = *ADIM*sizeof(CMUMPS_COMPLEX);
#if defined(_OPENMP)
#pragma omp atomic
#endif
  cmumps_size_async_pin = cmumps_size_async_pin + flops;
#if defined(DEBUG_GPU)
  printf(" <= GPU_ASYNC_PIN_MEMORY_C: %p of size %li, handle= %li\n",A, *ADIM*sizeof(CMUMPS_COMPLEX), handle);
#endif
}/* cmumps_gpu_async_pin_memory_C */
void MUMPS_CALL
...
#pragma omp atomic
#endif
  cmumps_wtime_async_pin = cmumps_wtime_async_pin + elapsed;
  elapsed = stop - cmumps_start_async_pin;
#if defined(_OPENMP)
#pragma omp atomic
#endif
  cmumps_wtime_pin = cmumps_wtime_pin + elapsed;
#if defined(DEBUG_GPU)
...
#endif
  handle = xkblas_unregister_memory_async((void *)A, *ADIM*sizeof(CMUMPS_COMPLEX));
  double flops = *ADIM*sizeof(CMUMPS_COMPLEX);
#if defined(_OPENMP)
#pragma omp atomic
#endif
  cmumps_size_async_unpin = cmumps_size_async_unpin + flops;
#if defined(DEBUG_GPU)
  printf(" <= GPU_ASYNC_UNPIN_MEMORY_C: %p of size %li, handle= %li\n",A, *ADIM*sizeof(CMUMPS_COMPLEX), handle);
#endif
}/* cmumps_gpu_async_unpin_memory_c */
#else
void MUMPS_CALL
CMUMPS_GPU_ASYNC_PIN_MEMORY_C(CMUMPS_COMPLEX *A, MUMPS_INT8 *ADIM)
{
  ...
  printf(" => GPU_ASYNC_PIN_MEMORY_C: %p of size %li \n",A, *ADIM*sizeof(CMUMPS_COMPLEX));
#endif
  handle = mumps_host_register_async((void *)A, *ADIM*sizeof(CMUMPS_COMPLEX));
  double flops = *ADIM*sizeof(CMUMPS_COMPLEX);
#if defined(_OPENMP)
#pragma omp atomic
#endif
...
CMUMPS_GPU_ASYNC_UNPIN_MEMORY_C(CMUMPS_COMPLEX *A, MUMPS_INT8 *ADIM)
{
  ...
  printf(" => GPU_ASYNC_UNPIN_MEMORY_C: %p of size %li \n",A, *ADIM*sizeof(CMUMPS_COMPLEX));
#endif
  handle = mumps_host_unregister_async((void *)A, *ADIM*sizeof(CMUMPS_COMPLEX));
  double flops = *ADIM*sizeof(CMUMPS_COMPLEX);
#if defined(_OPENMP)
#pragma omp atomic
#endif
...
CMUMPS_GPU_PIN_MEMORY_C(CMUMPS_COMPLEX *A, MUMPS_INT8 *ADIM)
{
  ...
  start = MPI_Wtime();
#if defined(USE_XKBLAS)
  error = xkblas_register_memory( A, *ADIM*sizeof(CMUMPS_COMPLEX) );
  if (error != 0)
  {
...
#else
  error = cudaHostRegister(A, *ADIM*sizeof(CMUMPS_COMPLEX), cudaHostRegisterPortable);
...
#pragma omp atomic
#endif
  cmumps_size_pin = cmumps_size_pin + flops;
#if defined(DEBUG_GPU)
  printf("<= GPU_PIN_MEMORY_C of %p of size %li, error =%i\n",A, *ADIM*sizeof(CMUMPS_COMPLEX), error);
#endif
}/* cmumps_gpu_pin_memory_c */
void MUMPS_CALL
CMUMPS_GPU_MODIFY_ARITH_TRHRESHOLD (MUMPS_INT *OLD, MUMPS_INT *NEW)
{
  /* used to temporarily modify and restore the ARITH_TRHRESHOLD 
     in: NEW value of cmumps_arith_threshold
     out: OLD current value of cmumps_arith_threshold
  */
  *OLD = cmumps_arith_threshold;
  cmumps_arith_threshold = *NEW;
}
void MUMPS_CALL
CMUMPS_GPU_SET_UNDER_L0_STATUS_C(MUMPS_INT *VALEUR)
{
  /* If cmumps_gpu_under_l0 is 1 it means we are under L0-OMP.
   * If it is 0 it means we are above L0-OMP or that there is no L0-OMP layer.
   */
  cmumps_gpu_under_l0 = *VALEUR;
  if (cmumps_gpu_under_l0 == 0)
  {
    cmumps_arith_threshold = cmumps_arith_threshold_above;
  }
  else
  {
    cmumps_arith_threshold = cmumps_arith_threshold_under;
  }
}
void MUMPS_CALL
CMUMPS_GPU_UNPIN_MEMORY_C(CMUMPS_COMPLEX *A, MUMPS_INT8 *ADIM)
{
  ...
  start = MPI_Wtime();
#if defined(USE_XKBLAS)
  error = xkblas_unregister_memory(A, *ADIM*sizeof(CMUMPS_COMPLEX));
  if (error != 0)
  {
...
#if defined(_OPENMP)
#pragma omp atomic
  cmumps_wtime_unpin = cmumps_wtime_unpin + elapsed;
#endif
#if defined(DEBUG_GPU)
  printf("<= GPU_UNPIN_MEMORY_C of %p of size %li, error=%d\n",A, *ADIM*sizeof(CMUMPS_COMPLEX),error);
#endif
}/* cmumps_gpu_unpin_memory_c */
#else
