...
void CoinPackedMatrix::reserve(const int newMaxMajorDim, const CoinBigIndex newMaxSize,
  bool create)
{
...
void CoinPackedMatrix::setDimensions(int newnumrows, int newnumcols)
{
...
void CoinPackedMatrix::deleteCols(const int numDel, const int *indDel)
{
  if (numDel) {
    if (colOrdered_)
...
void CoinPackedMatrix::deleteRows(const int numDel, const int *indDel)
{
  if (numDel) {
    if (colOrdered_)
      deleteMinorVectors(numDel, indDel);
...
/* Modify one element of packed matrix.  An element may be added.
   ...
   keepZero true */
void CoinPackedMatrix::modifyCoefficient(int row, int column, double newElement,
  bool keepZero)
{
...
void CoinPackedMatrix::removeGaps(double removeValue)
{
  if (removeValue < 0.0) {
...
void CoinPackedMatrix::swap(CoinPackedMatrix &m)
{
...
void CoinPackedMatrix::times(const double *x, double *y) const
{
  if (colOrdered_)
    timesMajor(x, y);
  else
...
//-----------------------------------------------------------------------------
#ifndef CLP_NO_VECTOR
void CoinPackedMatrix::times(const CoinPackedVectorBase &x, double *y) const
{
  if (colOrdered_)
    timesMajor(x, y);
  else
...
void CoinPackedMatrix::transposeTimes(const double *x, double *y) const
{
  if (colOrdered_)
    timesMinor(x, y);
  else
...
//-----------------------------------------------------------------------------
#ifndef CLP_NO_VECTOR
void CoinPackedMatrix::transposeTimes(const CoinPackedVectorBase &x, double *y) const
{
  if (colOrdered_)
    timesMinor(x, y);
  else
...
void CoinPackedMatrix::countOrthoLength(int *orthoLength) const
{
...
void CoinPackedMatrix::majorAppendSameOrdered(const CoinPackedMatrix &matrix)
{
  ...
  int i;
  if (majorDim_ + matrix.majorDim_ > maxMajorDim_ || getLastStart() + matrix.getLastStart() > maxSize_) {
...
void CoinPackedMatrix::timesMajor(const double *x, double *y) const
{
...
//-----------------------------------------------------------------------------
#ifndef CLP_NO_VECTOR
void CoinPackedMatrix::timesMajor(const CoinPackedVectorBase &x, double *y) const
{
  memset(y, 0, minorDim_ * sizeof(double));
...
void CoinPackedMatrix::timesMinor(const double *x, double *y) const
{
...
//-----------------------------------------------------------------------------
#ifndef CLP_NO_VECTOR
void CoinPackedMatrix::timesMinor(const CoinPackedVectorBase &x, double *y) const
{
  memset(y, 0, majorDim_ * sizeof(double));
  for (int i = majorDim_ - 1; i >= 0; --i) {
    double y_i = 0;
...
void CoinPackedMatrix::dumpMatrix(const char *fname) const
{
  if (!fname) {
...
bool CoinPackedMatrix::isEquivalent2(const CoinPackedMatrix &rhs) const
{
  ...
  if (isColOrdered() ^ rhs.isColOrdered()) {
    std::cerr << "Ordering " << isColOrdered() << " rhs - " << rhs.isColOrdered() << std::endl;
    return false;
  }
  if (getNumCols() != rhs.getNumCols()) {
    std::cerr << "NumCols " << getNumCols() << " rhs - " << rhs.getNumCols() << std::endl;
    return false;
  }
  if (getNumRows() != rhs.getNumRows()) {
    std::cerr << "NumRows " << getNumRows() << " rhs - " << rhs.getNumRows() << std::endl;
    return false;
  }
  if (getNumElements() != rhs.getNumElements()) {
    std::cerr << "NumElements " << getNumElements() << " rhs - " << rhs.getNumElements() << std::endl;
    return false;
  }
  ...
  for (int i = getMajorDim() - 1; i >= 0; --i) {
    ...
    if (!pv.isEquivalent(rhsPv, eq)) {
      std::cerr << "vector # " << i << " nel " << pv.getNumElements() << " rhs - " << rhsPv.getNumElements() << std::endl;
      int j;
      ...
      for (j = 0; j < pv.getNumElements(); ++j) {
        ...
        if (diff) {
          std::cerr << j << "( " << inds[j] << ", " << elems[j] << "), rhs ( " << inds2[j] << ", " << elems2[j] << ") diff " << diff << std::endl;
...
*/
bool CoinPackedMatrix::isEquivalent(const CoinPackedMatrix &rhs, const CoinRelFltEq &eq) const
{
  // Both must be column order or both row ordered and must be of same size
  ...
  const double *element, int numberOther)
{
  ...
  if (majorDim_ + number > maxMajorDim_ || getLastStart() + numberElements > maxSize_) {
    // we got to resize before we add. note that the resizing method
    // properly fills out start_ and length_ for the major-dimension
    // vectors to be added!
...
*/
    ...
    if (majStart < 0 || (majStart == (maxIndex + 1) && majLen != 0) || majStart > maxIndex + 1) {
      if (verbosity >= 1) {
        std::cout
          << "  " << majName << " " << majndx
...
*/
  for (CoinBigIndex ii = 0; ii <= maxIndex; ii++) {
    if (!((refCnt[ii] == 1 && inGap[ii] == false) || (refCnt[ii] == 0 && inGap[ii] == true))) {
      if (verbosity >= 1) {
        std::cout
...
*/
  ...
  if (errs > 0) {
    if (verbosity >= 1) {
      ...
      if (zeroes)
        std::cout << " (includes " << zeroes << " zeroes)";
      std::cout << "." << std::endl;
    }
  } else {
    if (verbosity >= 2) {
