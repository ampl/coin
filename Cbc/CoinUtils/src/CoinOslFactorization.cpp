...
{
  if (this != &other) {
    bool noGood = factInfo_.nrowmx != other.factInfo_.nrowmx && factInfo_.eta_size != other.factInfo_.eta_size;
  ...
  CoinBigIndex maximumU)
{
  ...
  CoinBigIndex size = static_cast< CoinBigIndex >(factInfo_.areaFactor * (maximumL + maximumU));
...
#endif
  if (!factInfo_.if_sparse_update && factInfo_.iterno > factInfo_.iter0 && numberRows_ >= C_EKK_GO_SPARSE && (factInfo_.lastEtaCount >> 2) < factInfo_.nrow && !factInfo_.switch_off_sparse_update) {
#if PRINT_DEBUG
...
//  preProcess.
void CoinOslFactorization::preProcess()
{
...
//Does factorization
int CoinOslFactorization::factor()
{
...
void CoinOslFactorization::makeNonSingular(int *sequence, int numberColumns)
{
...
void CoinOslFactorization::postProcess(const int *sequence, int *pivotVariable)
{
  factInfo_.iterin = factInfo_.iterno;
  factInfo_.npivots = 0;
  numberPivots_ = 0;
  const int *permute3 = factInfo_.mpermu + 1;
  assert(permute3 == reinterpret_cast< const int * >(factInfo_.kadrpm + numberRows_ + 1));
...
int CoinOslFactorization::updateColumn(CoinIndexedVector *regionSparse,
...
*/
int CoinOslFactorization::updateColumnFT(CoinIndexedVector *regionSparse,
  CoinIndexedVector *regionSparse2,
  bool /*noPermute*/)
{
  assert(numberRows_ == numberColumns_);
...
int CoinOslFactorization::updateTwoColumnsFT(CoinIndexedVector *regionSparse1,
  CoinIndexedVector *regionSparse2,
...
/* Updates one column (BTRAN) from regionSparse2
   ...
   Note - if regionSparse2 packed on input - will be packed on output
*/
int CoinOslFactorization::updateColumnTranspose(CoinIndexedVector *regionSparse,
...
int *CoinOslFactorization::numberInRow() const
{
  return reinterpret_cast< int * >(factInfo_.xrnadr + 1);
}
// Number of entries in each column
int *CoinOslFactorization::numberInColumn() const
{
...
// Returns pivot row
int *CoinOslFactorization::pivotRow() const
{
...
int *CoinOslFactorization::intWorkArea() const
{
  return reinterpret_cast< int * >(factInfo_.kw1adr);
}
// Returns permute back
int *CoinOslFactorization::permuteBack() const
{
  return factInfo_.kcpadr + 1;
}
// Returns array to put basis indices in
int *CoinOslFactorization::indices() const
{
  return factInfo_.xeradr + 1;
}
// Returns true if wants tableauColumn in replaceColumn
bool CoinOslFactorization::wantsTableauColumn() const
{
  return false;
}
...
*/
#ifdef CLP_REUSE_ETAS
void CoinOslFactorization::setUsefulInformation(const int *info, int whereFrom)
{
...
void CoinOslFactorization::clearArrays()
{
  factInfo_.nR_etas = 0;
  factInfo_.nnentu = 0;
  factInfo_.nnentl = 0;
...
void CoinOslFactorization::maximumPivots(int value)
{
...
static void c_ekksmem(EKKfactinfo *fact, int nrow, int maximumPivots)
{
  ...
  fact->nrow = nrow;
  if (!(nnetas > fact->last_eta_size || (!fact->xe2adr && fact->if_sparse_update) || nrow > fact->nrowmx || maximumPivots > fact->maxinv))
    return;
  ...
  if (nrowmx) {
    ...
    if (nnetas) {
	  ...
	  fact->R_etas_element = &XEEADR1()[kdnspt - 1];*/
        fact->R_etas_start = fact->xcsadr + (rhsFact->R_etas_start - rhsFact->xcsadr);
        fact->R_etas_index = fact->xeradr + (rhsFact->R_etas_index - rhsFact->xeradr);
        fact->R_etas_element = fact->xeeadr + (rhsFact->R_etas_element - rhsFact->xeeadr);
      }
    }
  }
